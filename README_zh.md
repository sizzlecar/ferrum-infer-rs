...
在 24 GB RTX 4090 上运行 functional 阶段的 27B 级 CUDA agent 模型。该 lane
已有正确性与并发证据，但仍有性能 known-gap，尚不是 release-grade：
...
27B CUDA functional 覆盖，并通过 L5 并发 gate 与同卡 llama.cpp sanity
floor；它尚未达到 release-grade 的 80% baseline 目标。证据目录：
...
- Gemma 3 27B GPTQ functional on CUDA，覆盖 tool-call、strict-schema、streaming、多轮和 c=32 client pressure gates；release-grade 性能仍是 known gap
