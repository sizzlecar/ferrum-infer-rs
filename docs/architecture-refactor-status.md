...
| [#145](https://github.com/sizzlecar/ferrum-infer-rs/pull/145) | CLI cleanup — `run_gguf_one_shot` removed | Delete the 1089-line `commands/run_gguf.rs` parallel inference path. `ferrum run <gguf>` now goes through `engine.infer_stream` like safetensors (registry's `LlmExecutorFactory` detects `WeightFormat::Gguf` and routes to `gguf_engine_loader`). Chat-profile env-var defaults (`KV_CAPACITY` 8192/512, `FERRUM_METAL_PAGED_KV`, MoE batched flags) move to `source_resolver::apply_gguf_chat_profile_env`, fired automatically when `resolve_model_source` returns a GGUF source under `Chat` autosize. Mac Metal Q8 GGUF 55.1 tok/s (+0.4%); CUDA regressions all within 3%. Diff: 4 files / +73 / -1101. |
| [#146](https://github.com/sizzlecar/ferrum-infer-rs/pull/146) | INT8 attn perf round 2 | `paged_decode_attention_int8.cu`: preload k/v scales to shmem in a one-shot block-cooperative prologue (collapses Step 3's 256-thread broadcast load of `v_scale` into 1 shmem read per kv_pos); fold `v_scale` into the softmax weight in-place after Step 2. Caller allocates 3× shmem. Cosine 0.99999 parity preserved. Qwen3-0.6B INT8 256-token TPOT: 24.72 → 22.82ms (-7.7%). INT8 within 5% of FP16 still open (architectural — single-token decode with per-element dequant; flash-attention-style tile fusion is a separate ~2-3 week project). |
| [#147](https://github.com/sizzlecar/ferrum-infer-rs/pull/147) | architectures → multimodal rename + stale comment cleanup | Audit #12 — `models/architectures/` only ever held multi-modal models (BERT, CLIP, Whisper, Qwen3-TTS); rename to `multimodal/` so `models/` (LLM hot path) and `multimodal/` (modality models) are visibly distinct. Plus stale "Phase 2 deferred" comments in `moe/layer.rs` and `moe/dispatch.rs::ExpertStack` cleaned (generic `moe_forward<B>` shipped Phase 3). 12 files renamed, 6 imports updated. Pure rename — workspace tests (330+) pass. |
| [#148](https://github.com/sizzlecar/ferrum-infer-rs/pull/148) | Audit #8 Phase 1 — split `cuda/mod.rs` | `cuda.rs` (4756 lines) → `cuda/mod.rs` + `cuda/int8_kv.rs`. Extract `BackendKvDtype<KvInt8>` marker, `OptionalCudaInt8` / `OptionalCudaScalesF16` lazy-alloc wrappers, `BackendInt8KvOps` impl, and `KvCacheQuant<CudaBackend, KvInt8>::new_paged_cuda`. Pure mechanical extraction. |
| [#149](https://github.com/sizzlecar/ferrum-infer-rs/pull/149) | Audit #8 Phase 2 — `cuda/{collective,graph}.rs` | `BackendCollective` impl (60 lines) → `cuda/collective.rs`. `BackendGraph` impl + `GraphSlotRaw` raw-FFI cuGraph wrapper + `DECODE_GRAPHS` multi-slot cache + invalidation helpers (~295 lines) → `cuda/graph.rs`. `decode_state_slot` bumped to `pub(super)`. mod.rs 4765 → 4408 (-7.5%). RTX 4090 Qwen3-0.6B FP16 smoke 77.6 tok/s. |
| [#150](https://github.com/sizzlecar/ferrum-infer-rs/pull/150) | Audit #8 Phase 3 — `cuda/quant.rs` | `BackendQuantMarlin` + `BackendQuantGguf` impls + `GptqStoreCuda` enum/alias + `use_triton_int4`/`use_vllm_moe` + `MarlinGatherScratch` + `moe_gemm_phase_fused_impl` + `marlin_gemm_with_perm` + `launch_vllm_marlin` (~1055 lines) → `cuda/quant.rs`. mod.rs 4412 → 3388 (-23%). Cumulative since Phase 0: -29%. RTX 4090 smoke 83.9 tok/s. |
| [#151](https://github.com/sizzlecar/ferrum-infer-rs/pull/151) | Audit #8 Phase 4 — `cuda/paged.rs` | `BackendPagedKv` impl + `SplitKScratch` process-global scratch + `paged_varlen_split_k_dispatch` + `paged_batched_flash_dispatch` + `paged_batched_decode_single_pass` (~1138 lines) → `cuda/paged.rs`. `BATCHED_SCRATCH_CAP` / `HOST_STAGING_TOTAL` bumped to `pub(super)`. mod.rs 3402 → 2298 (-32%). Cumulative -52%. RTX 4090 smoke 81.9 tok/s. |
| [#152](https://github.com/sizzlecar/ferrum-infer-rs/pull/152) | Audit #8 Phase 5 (final) — `cuda/moe.rs` | `BackendMoeFused` impl (`route_topk_softmax` + `try_gpu_route_topk_into_host` + `moe_align_block_size` + `moe_combine`, ~338 lines) → `cuda/moe.rs`. mod.rs 2302 → 1986 (-14%). **Cumulative since Phase 0: 4756 → 1986 (-58%).** What remains in mod.rs is core `impl Backend for CudaBackend` + `CudaState` + global stream/decode-state slots + `KvFp16` `BackendKvDtype` impl. RTX 4090 smoke 85.1 tok/s. |
| [#153](https://github.com/sizzlecar/ferrum-infer-rs/pull/153) | drop `simple_engine_config` | Same code-smell as #145 (`run_gguf_one_shot`): the SOLE config builder masquerading as a "simple" variant. Fold MVP defaults into per-section `Default` impls — `BatchConfig::max_batch_size` 16→32, `KvCacheConfig::max_blocks` 1024→2048 (or `FERRUM_KV_MAX_BLOCKS` env, set by GPU autosizer), `SchedulerConfig::max_running_requests` 256→32. Delete `simple_engine_config`. All 7 callers (3 CLI commands, 2 `TtsService` constructors, qwen concurrency test, lib unit tests) construct via `EngineConfig::default()` + 2 field assignments. RTX 4090 smoke 81.9 tok/s. |
