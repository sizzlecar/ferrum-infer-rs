...
5. 如果服务先达到 health ready 且尚未触发资源违规，必须完成一次固定的非流式
   `/v1/chat/completions` 真实生成，`max_tokens=1`，并保存请求、响应、HTTP 状态和时序；
6. 产品仍存活时真实观察到 `swap_growth` 或 physical headroom `<2 GiB`；
7. 保存逐样本 memory JSONL、原始 `vm_stat`/swap/pressure、PID/PGID/start identity、
   effective config、stdout/stderr/failure log 和 bounded receipt；
8. 最终 process group 完全回收，skip/waiver=`0`；
9. blocker 只解锁 G00 的 legacy fact freeze，不产生 M3 Metal performance baseline。
