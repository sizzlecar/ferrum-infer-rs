...
`6a596558` 和 binary SHA256 `e8d12cc...`。它把 decode attention 从
`5.826 ms/wave` 降到 `0.887 ms/wave`，profile-off c1 从历史 vNext
`35.5956 tok/s` 提升到 `39.5687 +/- 2.0459 tok/s`，但仍未达到预声明的
`40.935 tok/s` KEEP 线，更未达到 `76.158 tok/s` legacy 90% 线，因此状态为
REJECT。完整证据和下一 source-level hypothesis 记录在
[`G08B_QWEN35_35B.md`](G08B_QWEN35_35B.md)；在
`resource_prepare_attempt <=3.5 ms` 和 `submitted_wave_total <=13.5 ms` 的源码预测
成立前，不得启动下一次 paid M2 performance diagnostic。
