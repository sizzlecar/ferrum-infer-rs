...
可发布正确性和同机主流实现 80%，不得在最后发布阶段才第一次运行。
...
- same-host vLLM throughput ratio LCB `>=0.80`，每个 cell 都必须达到。
...
- 三模型全部新增/BLOCKED CUDA cell 的 TTFT/TPOT p95 均须 `<=1.25x` same-host vLLM，
  paired latency ratio 95% CI 上界 `<=1.25`；M1/M2/M3 一视同仁。legacy PASS cell 仍执行更严格的
...
无论 legacy 是否可用，vNext 必须达到 same-host vLLM LCB `>=0.80`。不能以缺 baseline 为由
只做功能 smoke。TTFT/TPOT p95 还必须 `<=1.25x vLLM`；全 paired request eligible 时
...
- 三模型 c1/c4/c16 throughput ratio LCB 全部 `>=0.80x llama.cpp`；external baseline 失败时
  必须修复 baseline，不能 waiver Ferrum cell。
- TTFT/TPOT p95 `<=1.25x llama.cpp`；全 paired request eligible 时 client-SSE-event ITL 也执行该门。
