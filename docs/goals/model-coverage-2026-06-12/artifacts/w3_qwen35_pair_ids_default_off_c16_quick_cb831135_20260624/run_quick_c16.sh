...
ROOT=/workspace/artifacts/w3_qwen35_pair_ids_default_off_c16_quick_cb831135_20260624
...

{
  ...
  echo lane=W3_QWEN35_PAIR_IDS_DEFAULT_OFF_C16_QUICK_CB831135
...
EXPECTED_SHA=cb831135c12f16cec1a3924178ce316c56f80813
...
server_config = json.loads((root / "server/effective_config.json").read_text())
run_config = json.loads((root / "run_smoke/effective_config.json").read_text())
def selected(decisions, name):
    for decision in decisions:
        if decision.get("selection") == name:
            return decision.get("selected")
    return None
metric["server_moe_decode_path"] = selected(server_config.get("decisions", []), "moe_implementation")
metric["run_moe_decode_path"] = selected(run_config.get("decisions", []), "moe_implementation")
for key in ("server_moe_decode_path", "run_moe_decode_path"):
    if metric[key] != "vllm_marlin_moe_device_route":
        raise SystemExit(f"unexpected {key}: {metric[key]}")
...
summary = {
    "lane": "W3_QWEN35_PAIR_IDS_DEFAULT_OFF_C16_QUICK_CB831135",
    "status": "passed",
...
(root / "summary.txt").write_text("W3 QWEN35 PAIR IDS DEFAULT OFF C16 QUICK DIAG PASS: " + str(root) + "\n")
PY
