...
SHA=517d49a01b5b46c2685400656aac8422529801db
...
ART=/workspace/artifacts/w3_qwen35_decode_backpressure_c32_517d49a0_${STAMP}
...
if last:
    ...
    for section in ['scheduler_before','scheduler_after_schedule','scheduler_after_process']:
        ...
        if isinstance(value, dict):
            summary['last_'+section]={k:value.get(k) for k in ['waiting_queue_len','prefill_queue_len','decode_queue_len','active_len','completed_total','failed_total','cancelled_total','admitted_total','capacity_deferred_total','capacity_backpressure_admit_limit','decode_backpressure_limit']}
    plan=last.get('plan')
