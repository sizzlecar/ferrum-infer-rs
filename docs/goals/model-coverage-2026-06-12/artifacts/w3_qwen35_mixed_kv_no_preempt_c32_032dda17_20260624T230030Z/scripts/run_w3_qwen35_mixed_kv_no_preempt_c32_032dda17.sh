...
SHA=032dda17739593e4352e87b2805e71363fb82ddd
...
ART=/workspace/artifacts/w3_qwen35_mixed_kv_no_preempt_c32_032dda17_${STAMP}
...
trap cleanup EXIT
log "lane_start art=$ART sha=$SHA release_evidence=false no_live_vllm=true cost_per_hr_approx=0.456"
{
  date -u
  hostname
  uname -a
...
find "$ART" -type f \( -name '*.log' -o -name '*.jsonl' \) ! -name '*.gz' -size +1M -print0 | xargs -0 -r gzip -f
log "FERRUM W3 QWEN35 MIXED KV NO PREEMPT C32 DIAG PASS: $ART"
