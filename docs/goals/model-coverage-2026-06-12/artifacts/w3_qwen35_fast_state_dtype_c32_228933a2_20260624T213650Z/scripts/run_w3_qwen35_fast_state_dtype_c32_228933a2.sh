...
SHA=228933a27fd1a4c2bc1943e9b7dcafa6e7b75714
...
ART=/workspace/artifacts/w3_qwen35_fast_state_dtype_c32_228933a2_${STAMP}
...
(cargo check -p ferrum-engine -p ferrum-scheduler -p ferrum-kv > "$ART/logs/cargo-check.stdout.log" 2> "$ART/logs/cargo-check.stderr.log"); echo $? > "$ART/env/cargo-check.exit"
...
assert data.get('selected_recurrent_state_max_slots') == 16, data.get('selected_recurrent_state_max_slots')
assert data.get('selected_admission_limit') == 16, data.get('selected_admission_limit')
print('ok')
PY
...
summary={"artifact":str(art),"sha":"228933a27fd1a4c2bc1943e9b7dcafa6e7b75714","bench_exit":(art/'perf/bench.exit').read_text().strip(),"release_evidence":False}
...
log "FERRUM W3 QWEN35 FAST STATE DTYPE C32 DIAG PASS: $ART"
