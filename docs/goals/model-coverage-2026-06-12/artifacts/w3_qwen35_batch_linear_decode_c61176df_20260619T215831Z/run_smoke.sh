...
on_exit() {
  ...
  if [[ $rc -eq 0 ]]; then
    STATUS=pass
    export STATUS
    echo "W3 QWEN35 VLLM H256 SMOKE PASS: ${ART}" | tee -a "${ART}/smoke.log"
  else
    STATUS=fail
    export STATUS
    echo "W3 QWEN35 VLLM H256 SMOKE FAIL rc=${rc}: ${ART}" | tee -a "${ART}/smoke.log" >&2
  fi
  python3 - <<PY || true
...
{
  "schema_version": 1,
  "lane": "W3 Qwen35 CUDA batch linear-attention decode smoke",
  "commit": "$(git rev-parse HEAD)",
  "source_fix_commit": "c61176df",
  "post_validation": "finish_reason length/repetition/tool/structured hard checks",
  "expected_runtime_minutes": "25-45",
  "hourly_cost_usd": 0.662962962962963,
  "stop_condition": "CUDA build failure, product smoke failure, diagnostic bench failure, or PASS artifact copied back",
