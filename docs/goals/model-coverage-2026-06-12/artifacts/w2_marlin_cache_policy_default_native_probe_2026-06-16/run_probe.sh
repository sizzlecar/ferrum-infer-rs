...
ART="docs/goals/model-coverage-2026-06-12/artifacts/w2_marlin_cache_policy_default_native_probe_2026-06-16"
...
if [ -x /tmp/gemma3_tail_mlp_cache_policy_legacy_plain ]; then
    sha256sum /tmp/gemma3_tail_mlp_cache_policy_legacy_plain > "$ART/probe/legacy_plain.sha256"
fi
if [ -x /tmp/gemma3_tail_mlp_cache_policy_product_default ]; then
    sha256sum /tmp/gemma3_tail_mlp_cache_policy_product_default > "$ART/probe/product_default.sha256"
fi
