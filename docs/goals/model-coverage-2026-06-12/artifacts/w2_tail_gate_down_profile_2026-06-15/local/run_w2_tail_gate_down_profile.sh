...
if [ -f /root/.cargo/env ]; then
  # Vast tmux sessions are not always login shells.
  . /root/.cargo/env
fi
export PATH="/root/.cargo/bin:${PATH}"

OUT=/workspace/w2_tail_gate_down_profile_2026-06-15
...
stop_condition: start/SSH/CUDA/source sync/build/server readiness first failure, tail gate/down profile c16/c32 small sample complete and copied, or 45min cap
correctness_gate: release build plus server readiness plus bench-serve --fail-on-error zero-error diagnostic
performance_command: bench-serve sharegpt natural ASCII, c16/c32, num_prompts=16, n_repeats=1, random-output-len=64, seed 9271, diagnostic only
TXT
...
{"cmd":["timeout","1800","target/release/ferrum","bench-serve","--base-url","http://127.0.0.1:$PORT","--model","gemma3:27b-gptq","--tokenizer","$TOKENIZER","--dataset","sharegpt","--sharegpt-path","$DATASET","--random-output-len","64","--concurrency-sweep","16,32","--num-prompts","16","--n-repeats","1","--fail-on-error","--seed","9271","--out","$OUT/profile/bench_sharegpt_c16_c32_16x1_tail_gate_down_profile.json"]}
JSON
...
timeout 1800 target/release/ferrum bench-serve \
  ...
  --fail-on-error \
  --seed 9271 \
  --out "$OUT/profile/bench_sharegpt_c16_c32_16x1_tail_gate_down_profile.json" \
...
pat = re.compile(
    ...
    r"tail_down=(\\d+)us\\((\\d+)\\) tail_act=(\\d+)us\\((\\d+)\\) "
    ...
    rows.append(
        {
            ...
            "tail_act_us": int(match.group(21)),
            "tail_act_calls": int(match.group(22)),
            "tail_resid_us": int(match.group(23)),
            "tail_resid_calls": int(match.group(24)),
            "unwrapped_us": int(match.group(25)),
        }
    )
