...
REMOTE_ART=/workspace/ferrum-artifacts/w2_tail_mlp_chain_native_probe_2026-06-16
...
OUT_BIN="$REMOTE_ART/gemma3_tail_mlp_chain_perf" \
  timeout 20m bash scripts/microbenches/build_and_run_gemma3_tail_mlp_chain_perf.sh \
  > "$REMOTE_ART/probe.stdout" 2> "$REMOTE_ART/probe.stderr"
rc=$?
printf '%s\n' "$rc" > "$REMOTE_ART/probe.rc"
if [ "$rc" -eq 0 ]; then
  sha256sum "$REMOTE_ART/gemma3_tail_mlp_chain_perf" > "$REMOTE_ART/gemma3_tail_mlp_chain_perf.sha256"
fi
