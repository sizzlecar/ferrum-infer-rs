# W2 Gemma3 FA2 Source Full Config Product Smoke GPU Contract

- Lane: W2 Gemma3 typed FA2-source full-config product smoke.
...
- Correctness gate: `ferrum serve` from artifact-local complete `ferrum.toml`
  copied from the repo root, with `runtime.use_vllm_paged_attn=true`,
  `runtime.fa2_source=true`, and `runtime.fa2_direct_ffi=false`; decision trace
  must select
