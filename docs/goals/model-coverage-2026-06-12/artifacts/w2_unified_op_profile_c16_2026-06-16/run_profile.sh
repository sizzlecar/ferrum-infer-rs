...
OUT=${OUT:-/workspace/artifacts/w2_unified_op_profile_c16_2026-06-16}
export OUT
...
p = root / "serve_smoke_kv512/chat_stream.sse"
s = p.read_text(errors="replace")
ok = "data: [DONE]" in s and "5" in s
(root / "serve_smoke_kv512/smoke.ok").write_text(str(ok))
print("SMOKE_OK", ok)
if not ok:
    print(s[:2000])
    sys.exit(2)
...
grep -nE \
  'first-token-prof|stream-ttft-prof|unified-op-profile|unified-prof|iter-prof|unified-decode|panic|CUDA_ERROR|illegal|OOM|out of memory|error' \
