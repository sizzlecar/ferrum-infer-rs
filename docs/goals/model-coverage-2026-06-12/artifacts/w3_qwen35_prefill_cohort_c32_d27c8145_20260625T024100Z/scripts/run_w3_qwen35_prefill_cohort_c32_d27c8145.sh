...
SHA=d27c8145a050eac77d041a520dc928757546137f
CODE_CANDIDATE_SHA=311e51ad
...
PORT=55995
THROUGHPUT_FLOOR=458.066

STAMP=$(date -u +%Y%m%dT%H%M%SZ)
ART=/workspace/artifacts/w3_qwen35_prefill_cohort_c32_d27c8145_${STAMP}
...
log "lane_start art=$ART sha=$SHA code_candidate=$CODE_CANDIDATE_SHA release_evidence=false no_live_vllm=true cost_per_hr_approx=0.478 hypothesis=prefill_first_default_16_avoids_full_32_prefill_wave floor=$THROUGHPUT_FLOOR"
{
  date -u
  hostname
  uname -a
...
(cargo check -p ferrum-kernels -p ferrum-models -p ferrum-cli > "$ART/logs/cargo-check.stdout.log" 2> "$ART/logs/cargo-check.stderr.log")
echo $? > "$ART/env/cargo-check.exit"
...
RUN_CMD=(target/release/ferrum run "$MODEL" --backend cuda --gpu-devices 0
  --gpu-memory-utilization 0.90 --max-num-seqs 32 --kv-capacity 512
  --max-num-batched-tokens 192 --temperature 0 --max-tokens 8
  --output-format jsonl --prompt 'What is 2+3? Answer with only the number.'
...
policy=(data.get('admission') or {}).get('scheduler_policy')
assert policy == 'prefill_first_until_active:16+prefill_step_chunk:6', policy
print('ok')
PY_RUN_ASSERT
log ferrum_run_smoke_done

log serve_start
SERVE_CMD=(target/release/ferrum serve "$MODEL" --host 127.0.0.1 --port "$PORT"
...
log bench_c32_start
BENCH_CMD=(target/release/ferrum bench-serve --base-url "http://127.0.0.1:${PORT}"
  --model "$MODEL" --tokenizer "$TOKENIZER" --dataset sharegpt
  --sharegpt-path "$DATASET" --random-output-len 128 --ignore-eos
  --concurrency 32 --num-prompts 32 --warmup-requests 4 --n-repeats 1
  --fail-on-error --seed 9271 --timeout 600
...
set +e
python3 - "$ART" "$SHA" "$THROUGHPUT_FLOOR" <<'PY_SUMMARY'
import gzip, json, pathlib, re, sys
art=pathlib.Path(sys.argv[1]); sha=sys.argv[2]; floor=float(sys.argv[3])
summary={"artifact":str(art),"sha":sha,"bench_exit":(art/'perf/bench.exit').read_text().strip(),"release_evidence":False,"no_live_vllm":True,"throughput_floor":floor}
bench=art/'perf/bench_ferrum_c32_32x1.json'
throughput=None; completed=None; errored=None
if bench.exists():
    try:
        data=json.load(open(bench))
        summary['output_token_count_source']=data.get('output_token_count_source')
        summary['completed_per_run']=data.get('completed_per_run')
        summary['errored_per_run']=data.get('errored_per_run')
        for key in ['output_throughput_tps','total_throughput_tps','request_throughput_rps','ttft_ms','tpot_ms','itl_ms']:
            summary[key]=data.get(key)
        value=data.get('output_throughput_tps')
        throughput=value.get('mean') if isinstance(value, dict) else value
        completed=data.get('completed_per_run')
        errored=data.get('errored_per_run')
    except Exception as e:
        summary['bench_parse_error']=str(e)
trace=art/'server/scheduler_trace.jsonl'
rows=[]
if trace.exists():
    fh=open(trace, 'rt', errors='replace')
elif (art/'server/scheduler_trace.jsonl.gz').exists():
    fh=gzip.open(art/'server/scheduler_trace.jsonl.gz','rt',errors='replace')
else:
    fh=[]
for line in fh:
    if line.startswith('{'):
...
summary['scheduler_trace_lines']=len(rows)
if rows:
    summary['last_iteration']=rows[-1].get('iteration')
    for section in ['scheduler_before','scheduler_after_schedule','scheduler_after_process']:
        value=rows[-1].get(section)
    ...
    full_prefill_32=mixed=decode_only=0
    for row in rows:
        plan=row.get('plan') or {}
        p=plan.get('prefill_items') or 0
        d=plan.get('decode_items') or 0
        b=plan.get('batch_size') or 0
        if p==32 and d==0 and b==32:
            full_prefill_32 += 1
        if p and d:
            mixed += 1
        if d and not p:
            decode_only += 1
    summary['trace_shape']={'full_prefill_32_iterations':full_prefill_32,'mixed_iterations':mixed,'decode_only_iterations':decode_only}
...
if summary['bench_exit'] != '0':
    rejected=True; reasons.append('bench_exit_nonzero')
if throughput is None:
    rejected=True; reasons.append('missing_output_throughput')
elif throughput < floor:
    rejected=True; reasons.append(f'output_throughput_below_floor:{throughput:.3f}<{floor:.3f}')
if completed != [32]:
    rejected=True; reasons.append(f'completed_per_run:{completed}')
if errored != [0]:
    rejected=True; reasons.append(f'errored_per_run:{errored}')
if summary['log_counts']['oom_mentions'] or summary['log_counts']['panic_mentions'] or summary['log_counts']['cancelled_during_decode']:
    rejected=True; reasons.append('blocking_log_counts')
summary['diagnostic_rejected']=rejected
summary['reject_reasons']=reasons
(art/'perf/bench_summary.json').write_text(json.dumps(summary, indent=2, sort_keys=True)+'\n')
print(json.dumps(summary, indent=2, sort_keys=True))
sys.exit(52 if rejected else 0)
PY_SUMMARY
summary_rc=$?
set -e
...
if [ "$summary_rc" != 0 ]; then
  log "bench_c32_rejected summary_exit=$summary_rc"
  exit "$summary_rc"
fi
log "FERRUM W3 QWEN35 PREFILL COHORT C32 DIAG PASS: $ART"
