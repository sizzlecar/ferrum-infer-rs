...
export NO_COLOR=1
ART="/workspace/artifacts/w3_qwen35_recurrent_admission_c32_ce6ef62d_20260624T131658Z"
...
cd "$REPO"
{
  ...
  echo lane=W3_QWEN35_RECURRENT_ADMISSION_C32_DIAG
...
for r in rows:
    print("RECURRENT_ADMISSION_C32_DIAG", "c", r.get("concurrency"), "output_tps", r.get("output_throughput_tps",{}).get("mean"), "completed", r.get("completed_per_run"), "errored", r.get("errored_per_run"), "ttft_p95", r.get("ttft_ms",{}).get("p95",{}).get("mean"), "itl_p95", r.get("itl_ms",{}).get("p95",{}).get("mean"))
PY
python3 - <<PY
import json
...
summary={
    ...
    "result_counts": {},
    "max_active_len": 0,
    "max_admitted_total": 0,
    "max_cancelled_total": 0,
    "max_completed_total": 0,
    "max_generated_tokens_seen_in_prefill": 0,
    "prefill_with_generated_tokens_iterations": 0,
    "prefill_with_generated_tokens_observations": 0,
    "serve_log_counts": {},
}
results=Counter()
if trace.exists():
...
echo "FERRUM W3 QWEN35 RECURRENT ADMISSION C32 DIAG PASS: $ART" | tee "$ART/summary.txt"
