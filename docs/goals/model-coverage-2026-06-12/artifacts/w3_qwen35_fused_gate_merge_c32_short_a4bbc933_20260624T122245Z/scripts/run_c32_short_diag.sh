...
export NO_COLOR=1
ART="/workspace/artifacts/w3_qwen35_fused_gate_merge_c32_short_a4bbc933_20260624T122245Z"
...
cd "$REPO"
{
  ...
  echo lane=W3_QWEN35_FUSED_GATE_MERGE_C32_SHORT_DIAG
...
for r in rows:
    print("FUSED_GATE_MERGE_C32_DIAG", "c", r.get("concurrency"), "output_tps", r.get("output_throughput_tps",{}).get("mean"), "completed", r.get("completed_per_run"), "errored", r.get("errored_per_run"), "ttft_p95", r.get("ttft_ms",{}).get("p95",{}).get("mean"), "itl_p95", r.get("itl_ms",{}).get("p95",{}).get("mean"))
PY
echo "FERRUM W3 QWEN35 FUSED GATE MERGE C32 SHORT DIAG PASS: $ART" | tee "$ART/summary.txt"
