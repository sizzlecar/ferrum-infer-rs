...
OUT=/workspace/w2_tail_profile_buckets_2026-06-15
...
stop_condition: start/SSH/CUDA/source sync/build/server readiness first failure, tail bucket profile c16/c32 small sample complete and copied, or 45min cap
...
{"cmd":["timeout","1800","target/release/ferrum","bench-serve","--base-url","http://127.0.0.1:$PORT","--model","gemma3:27b-gptq","--tokenizer","$TOKENIZER","--dataset","sharegpt","--sharegpt-path","$DATASET","--random-output-len","64","--concurrency-sweep","16,32","--num-prompts","16","--n-repeats","1","--fail-on-error","--seed","9271","--out","$OUT/profile/bench_sharegpt_c16_c32_16x1_tail_profile.json"]}
JSON
...
timeout 1800 target/release/ferrum bench-serve \
  ...
  --fail-on-error \
  --seed 9271 \
  --out "$OUT/profile/bench_sharegpt_c16_c32_16x1_tail_profile.json" \
...
pat = re.compile(
    ...
    r"tail_mlp=(\\d+)us\\((\\d+)\\) tail_act=(\\d+)us\\((\\d+)\\) "
    ...
    rows.append(
        {
            ...
            "tail_act_us": int(match.group(17)),
            "tail_act_calls": int(match.group(18)),
            "tail_resid_us": int(match.group(19)),
            "tail_resid_calls": int(match.group(20)),
            "unwrapped_us": int(match.group(21)),
        }
    )
