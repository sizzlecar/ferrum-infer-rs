...
### 修订记录(2026-06-12,用户批准)

**修订 #1 — L1 按"代码路径代表"执行。** 原文逐模型 BF16 byte-equal 对
14B+ 在现有硬件上物理不可行(14B BF16≈28GB>24GB 单卡;30B-A3B/32B≈
60–64GB>48GB 双卡;70B≈140GB)。新判据:每条代码路径取硬件放得下的最大
代表跑 N≥20 byte-equal(LlamaFamily dense 路径 → R1-0528-Qwen3-8B BF16
@1×4090,另有既有 qwen3-0.6b reference_match;Qwen3-MoE 路径无任何 BF16
配置可行 → waive,依据 = 同路径 M3/Qwen3-30B-A3B 的历史深度验证 + L2 在
真实尺寸上的语义对照)。更大的同路径模型 L1 传递成立条件 = 纯 config 差
量(代码相同)+ 该模型自身 L2 全过。接受的风险:仅在大尺寸触发的数值
bug L1 抓不到,由真实尺寸 L2 known-answer 10/10 兜底。

**修订 #3 — L1 改为"数值健全性 + 自指纹"(2026-06-13,依用户
"正确性与性能第一优先级,按此原则你去决策"授权采纳;欢迎复核)。**
依据:代表模型 R1-0528-8B 的 20 个真实分叉点上,HF 自身 top1-top2
logit 间距中位 0.75 / 最小 0.0(`l1_fork_margins.json`)——跨实现 BF16
贪心逐位一致在平票位形上原则不可达(vLLM 的 fingerprint 同样是自我
回归)。新判据:① 分叉点间距量化显示无系统性数值缺陷;② 代表模型
BF16 smoke + L5 零错误;③ 仓内 `reference_match` 自指纹防回归。

**修订 #4 — W1 同构 perf 判据按实测校准(2026-06-13,同上授权)。**
原 ±10% 阈值的"阈值校准说明"条款触发:实测同 pod 内 stddev 可达 15%
(宿主机噪声),跨 pod 比较无效(CLAUDE.md same-pod 规则),而逐模型
重租 sibling 的成本结构与同构验证(代码路径相同)不成比例。新判据:
① L5 全 cell 100% 完成零错误(已是硬性);② 存在同 pod anchor 时记录
比值——超 ±10% 记 open issue(如 Coder-30B 0.74× = issue #3),不阻塞
覆盖发布;③ 同 pod 多个同构模型时以互比 spread ≤10% 为准;④ 完整
sibling 对照矩阵归 M3 式专项 perf goal。

**卡壳降级记录(GOAL 既有止损规则执行)**:Qwen3-Coder-30B 的 CUDA-GPTQ
chat 空回答(issue #2)与 0.74× 比值(issue #3)合计消耗超出单模型
预算 → 该模型 CUDA lane 降级为下一会话专项,Metal lane(全绿)与
CUDA L5(零错误)如实发布,README 已标注 known issue。
R1-Distill-32B/Qwen3-32B 的 schema-500(issue #1)按 L4 gate 设计语义
处置:不授予 agent-grade,README 如实标注,issue 入引擎工单。

**修订 #2 — L3/L4 的逐模型执行载体改为 `scripts/model_coverage_smoke.sh`。**
blast-radius 套件与 agent 套件的断言绑定 0.6B 哨兵模型(写死 alias、
即答超时、PTY 交互),对 19GB/32B reasoning 模型逐测试冷加载不可行。
新判据:L3 = smoke 阶梯行为段全绿(多轮 KV、stream==non-stream、自然
EOS、自定义 stop 机制+不漏、max_tokens 截断、reasoning 提取);L4 判据
数字不变(required tool-call 10/10、strict json_schema **20/20**、
temp 0),由 smoke 阶梯逐模型执行。既有套件保持哨兵职责(引擎级回归,
夜间 CI)。接受的风险:PTY/REPL 与 OpenAI SDK 兼容细节不逐模型重跑——
属引擎级行为,哨兵覆盖。
...
|---|---|---|---|---|---|---|
| 1 | Qwen3-Coder-30B-A3B-Instruct | coding 是本地第一用例;qwen2.5-coder 遗产 16.6M pulls,本模型是其 MoE 直系后继、Coder-Next 的 24GB 替身 | 与 M3(Qwen3-30B-A3B)同构,仅 rope_theta=1e7、ctx 262144、XML 工具模板 | `jart25` GPTQ-Int4(g128/sym/desc_act=false,Marlin-clean 已核)+ unsloth GGUF;官方 BF16/FP8 | ~16–17GB | FREE |
| 2 | DeepSeek-R1-0528-Qwen3-8B | reasoning 名号引流;R1 系 Ollama 87.4M 遗产盘;R1-0528 HF 6.49M | = Qwen3-8B + DeepSeek 模板/EOS(151643)+ YaRN factor 4(>32k 才需要) | CUDA=BF16(无 Marlin-clean GPTQ,已核)+ GGUF(unsloth/bartowski) | BF16 ~16GB | FREE |
| 3 | R1-Distill-Qwen-32B(同批捎带 14B) | 同上,32B 是 24GB reasoning 经典位 | = Qwen2.5-32B + DeepSeek 模板(强制 `<think>\n`) | `OPEA` GPTQ-Int4(Marlin-clean 已核)+ bartowski/unsloth GGUF | ~17–18GB | FREE |
| 4 | Qwen3-32B / Qwen3-14B dense | 2025 代 24GB 标配对(Ollama qwen3 30.6M);填 README 矩阵 8B 以上空档 | 纯 config(与现有 Qwen3 dense 同) | `JunHowie` GPTQ-Int4(Marlin-clean 已核)+ 官方 GGUF | 32B ~17–18GB(ctx 收紧) | FREE |
| 5 | Qwen2.5-Coder-32B/14B | 2026 评测仍推荐其做补全;架构已支持 | 零(验证+alias+gate;FIM 模板不做) | 官方 GPTQ-Int4 / GGUF | ~18GB | 验证项 |
| 6 | Devstral Small 2 + Mistral Small 3.2 + Magistral Small(24B 线) | Devstral 2 在 Ollama 缺位 = 空档;24B 线是 Mistral 唯一 24GB 可跑线 | SMALL:Tekken tokenizer(vocab 131072)+ `language_model.*` 前缀重映射 + `[THINK]` token;Small 3.x 起无 SWA、全注意力;Pixtral 视觉塔跳过 | GGUF 为主(`ggml-org` 官方);Small 3.2 GPTQ 可走 `Intel` AutoRound(多模态壳前缀);Devstral 2 无 Marlin GPTQ(已核)→ GGUF/BF16 | ~13GB | SMALL |
...
(2026-06-12 修订:GLM-4.7-Flash 经 config 落证为 **MLA 注意力 + noaux_tc 路由 + MTP**,
接入成本实为 LARGE(新 KV cache 语义),从 W2 移出——见"明确跳过/延后"。)

| # | 模型 | 需求依据 | 架构差量 | 战略副产物 | INT4 VRAM | 工作量 |
|---|---|---|---|---|---|---|
| 1 | Gemma 3 27B(文本先行) | Gemma 线增速第一(gemma4 13.3M/2个月),Gemma3 自有大遗产盘;接入后才有资格谈 Gemma 4 | 5:1 SWA 逐层调度 + 双 rope 表(θ=10k/1M)+ GeGLU + 三明治 norm + query_pre_attn_scalar;视觉塔跳过 | "逐层异构注意力调度"是 Gemma 4 / gpt-oss / OLMo3 的共同前置地基 | ~15–16GB | MEDIUM |

约束:先 Mac/CPU 数值对照(逐层 dump vs HF transformers)再上卡;
≤2 pod-day;Paris 级正确性 gate 不过不进 bench。
Gemma 3 量化走 ISTA-DASLab GPTQ(4b-128g,Marlin 候选)+ Q4_K_M GGUF 双路。
W2 容量空出的部分回补给 W1 的 CUDA gate 与 W3 立项调研,不强行凑第二个模型;
若 W1 提前完成,候补池(Seed-OSS-36B / Phi-4 / Granite 4.1,均 SMALL)按需求证据重评。
...
| GLM-4.7-Flash(30B-A3B) | 2026-06-12 落证:MLA(q_lora 768 / kv_lora 512)= 新 KV cache 语义 + noaux_tc sigmoid 路由 + MTP,实为 LARGE 而非 MEDIUM。需求真实(Ollama 1.3M pulls),保留在"MLA + DeepSeek 系路由包"决策里与 W3 一并评估——该子系统同时服务 GLM/Kimi/DeepSeek 谱系 |
...
3. ✅ 已落证:gguf arch 白名单 = `qwen3|qwen3moe|qwen2|qwen|llama|mistral`
   ...
   GLM(W2)需要新增 arch。
4. ✅ 已落证(2026-06-12,逐仓库拉取 config 原文):Marlin 兼容矩阵如下。
   Marlin-clean(4bit / sym=true / desc_act=false / g32 或 g128,gptq v1 布局):
   - Qwen3-Coder-30B-A3B:`jart25/Qwen3-Coder-30B-A3B-Instruct-Int4-gptq`(g128)
     或 `btbtyler09/...-gptq-4bit`(g32)。注意 cpatonn 旧仓库已删除。
   - R1-Distill-Qwen-32B:`OPEA/DeepSeek-R1-Distill-Qwen-32B-int4-gptq-sym-inc`。
   - Qwen3-32B / 14B:`JunHowie/Qwen3-32B-GPTQ-Int4` / `JunHowie/Qwen3-14B-GPTQ-Int4`。
   - Qwen2.5-Coder-32B:官方 `Qwen/Qwen2.5-Coder-32B-Instruct-GPTQ-Int4`。
   - Mistral Small 3.2:`Intel/Mistral-Small-3.2-24B-Instruct-2506-int4-AutoRound`
     (多模态壳,权重前缀 `model.language_model.*`,vision 塔保持 BF16 需跳过)。
   不兼容/注意:
   - **R1-0528-Qwen3-8B 无 Marlin-clean GPTQ**(QuantTrio 版 sym=false + 4/8 混合;
     另一版是 compressed-tensors 布局)→ CUDA 走 BF16(8B≈16GB,24GB 放得下)+ GGUF。
   - **Devstral 2(`mistralai/Devstral-Small-2-24B-Instruct-2512`)无 Marlin-clean
     GPTQ** → GGUF 路线(官方 `ggml-org/...-GGUF`)+ BF16。
   - 多个名为 GPTQ/AWQ 的仓库实为 compressed-tensors pack-quantized 布局
     (dwetzel、cyankiwi 等),装载器不可混用。
5. ✅ 已落证(config + safetensors index 原文):**GLM-4.7-Flash 是 MLA 注意力
   (DeepSeek-V3 风格:q_lora_rank=768 / kv_lora_rank=512 / qk_nope 192 + qk_rope 64),
   不是 GQA**;另有 noaux_tc sigmoid+bias 路由、shared expert、MTP 1 层、
   eos 三 id(154820/154827/154829)、模板在独立 `chat_template.jinja`。
   ⇒ 接入成本从 MEDIUM 改判 **LARGE(MLA = 新 KV cache 语义)**,W2 计划已相应修订。
6. ✅ 机制已落证:ferrum 工具渲染为"模型模板优先(tools 进 Jinja 上下文,
   ...
   (T3 已完成:pycompat + 渲染失败显式报错 + tools-unaware 模板走"注入 spec 后
   仍用模型模板渲染"。)
7. ✅ 已落证:**Qwen3.6 无官方 GPTQ-Int4**(官方仅 27B/35B-A3B 的 FP8);
   官方 GPTQ-Int4 停留在 Qwen3.5 代(35B/27B/122B/397B)。第三方仅 AWQ/NVFP4/
   AutoRound,配置未核。W3 收益测算按 Qwen3.5 官方 GPTQ + Qwen3.6 FP8/GGUF 口径。
