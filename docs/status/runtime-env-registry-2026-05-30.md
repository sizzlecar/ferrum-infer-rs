...
- `FERRUM_KV_MAX_BLOCKS` and `FERRUM_MAX_BATCHED_TOKENS` are now applied from
  the runtime snapshot into typed `EngineConfig` fields. `KvCacheConfig` and
  `BatchConfig` defaults no longer read process env directly; `ferrum run`,
  `ferrum bench`, and `ferrum serve` own the explicit runtime snapshot parse.
...
- The backend timer helper now takes an already-resolved boolean gate instead
  ...
  timer.
- The single-process `ferrum bench` command now captures a startup
  `RuntimeConfigSnapshot` before CUDA device reporting and tensor-parallel
  display. It reads `FERRUM_CUDA_GRAPH` and `FERRUM_TP` from that snapshot
  instead of calling `std::env::var` directly.
- `ferrum run`, `ferrum bench`, and `ferrum serve` now resolve
