...
截至 `5083aab5`，该检查点的集成工作树默认 workspace 测试为 5884 通过、72 忽略，fmt/check/clippy 和 Metal 编译通过；
另有 128 项 planner、95 项 Metal 控制器测试通过，包含真实 partial→final、decode、mixed 与最终首波证据交付的执行对照。
...
A2 的独立新校准仍有 59 次 Unknown 和 44 次低估；原始 8 请求 C8 Enforce 诊断完整输出 1608 tokens、0 请求错误，
找到 51 次完整共同计划但实际完整计划提交仍为 0。发布预算耗尽恢复和最终重放首波证据交付已提交，性能待新构建测量。
A1/A2 与上述发布路径改造继续使用各自固定构建单列验证；后续 CUDA/A3 集成不能继承这个检查点的测试结论。
