...
| `tools` | Partially supported | Function tool definitions parse and are carried through the structured request boundary; non-function tool types return HTTP 400 with `param=tools`. |
| `tool_choice=auto/none` | Supported | Parsed and carried through request metadata. |
| forced `tool_choice` / `required` | Rejected | Tool-call generation is not implemented yet, so forced selection returns HTTP 400. |
| legacy `functions` / `function_call=auto/none` | Supported | Parsed for SDK compatibility and carried through structured request data. Assistant `function_call` responses serialize in the legacy OpenAI shape. |
