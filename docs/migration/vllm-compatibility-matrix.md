...
| `--enable-prefix-caching` | Supported as product cache request | G1/G3 gates verify the alias, effective config, `/health`/`/metrics`, and real engine-level `position=real-kv-reuse` hits on supported decoder models |
| `--no-enable-prefix-caching` | Supported as runtime flag | G1/G3 gates verify the alias and disabled hit counters stay at zero |
