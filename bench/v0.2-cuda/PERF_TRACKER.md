...
| 2026-05-13 | `388917e` | Phase 3: token-budget scheduler + scratch prewarm (M3 c=32) | **1076 avg** (1101 / 1052) | **58%** | — | — | — | — | scratch.max_tokens preallocated to 2048 (was 1, lazy-grown to ~200) via `FERRUM_MAX_BATCHED_TOKENS=2048` set by autosize. Scheduler `max_prefill_batch=8` cap dropped — sole budget is `hint.max_tokens`. Gate `m_total > scratch.max_tokens` no longer fires for cohort prefills; unified_forward activates. **TTFT_p50 614 → 310 ms (−50%) is the direct unified-prefill signal**. Also fixed: paged split-K transient OOM (`total_q_tokens > 64` skip), autosize early-return for KV-set-by-user case. Known issue: multi-cell sweep at c=32 hits `paged_batched_flash phase1: INVALID_VALUE` from MOE_GRAPH × scratch-realloc interaction (standalone c=32 OK). |
| 2026-05-13 | `388917e` | Phase 3 (M3 c=16) | **875** | **71%** (vLLM 1232) | — | — | — | — | Larger gain at mid-concurrency: 788 → 875 = **+11.0%**. At c=16 the cohort window is more porous (decodes finish at staggered points), giving the unified path more mix opportunities per iter than c=32 cohort-bursts. |
| 2026-05-13 | `388917e` | Phase 3 (M2 c=32) | 887 avg | 41% | — | — | — | — | Across 3 runs (872 / 889 / 896 / 886 / 878): **+1% vs 881 baseline (noise)**. TTFT_p50 614 → 318 ms (−48%) confirms unified prefill is firing; throughput is decode-bound (Marlin dense GEMM at m=32 dominates). |
| | | | | | | | | | |
