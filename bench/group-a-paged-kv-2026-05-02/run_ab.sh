...
run_one_phase() {
  ...
  # NOTE: per_token mode also gets FERRUM_METAL_PAGED_KV=1 from the
  # outer env — that's the SAME paged-KV path now used by the new m=1
  # branch in forward_layer. The only behavioural delta between modes
  # is whether forward_layer_batched_decode runs at m=N (batched) or
  # decode_batch's outer gate falls through to the per-token loop.

  echo "starting ferrum serve (env: $mode_env) → $server_log"
  # Pool sizing: bench harness uses fresh cache_id per request, so the
  # paged pool needs headroom = max_concurrency + slack for in-flight
  # release. Default to 2× concurrency. KV_CAPACITY at 512 keeps total
  # pool memory under 4 GB at MAX_SEQS=32 (16384 KV slots × 2 × 4 KV
  # heads × 128 head_dim × 4 bytes × 48 layers ≈ 3.1 GB).
  env \
