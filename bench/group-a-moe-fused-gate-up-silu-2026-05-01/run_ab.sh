#!/bin/bash
# A/B bench: ferrum serve on Qwen3-30B-A3B Q4_K_M with fused gate+up+silu
# kernel ON vs OFF. Runs c=1, 4, 8, 16 in burst mode using the standard
# vLLM-style bench_serving.py harness.
#
# Outputs JSON per (mode, concurrency) into this directory.
#
# Usage:
#   ./run_ab.sh                          # full sweep
#   ./run_ab.sh 8                        # only c=8
#
# Mode toggle:
#   FUSED ON  : default (FERRUM_MOE_FUSED_GATE_UP_SILU unset)
#   FUSED OFF : FERRUM_MOE_FUSED_GATE_UP_SILU=0
#
# Notes:
#   - Each phase serves the model fresh (load takes ~30-60s on M1 Max)
#     so the warmup state is the same across modes.
#   - Server is killed between concurrencies — paged-KV pool size is set
#     at startup, can't be resized live.
...
run_one_phase() {
  ...
  # Server env: paged KV, fused MoE, decode-op profile to capture per-stage times.
  local fused_env=""
  if [ "$mode" = "unfused" ]; then
    fused_env="FERRUM_MOE_FUSED_GATE_UP_SILU=0"
  fi

  echo "starting ferrum serve (env: $fused_env) → $server_log"
  env \
