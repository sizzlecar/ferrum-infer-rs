...
- Pass `--test-threads=1` to bounded libtest-only commands. For the bench-inclusive `cargo test --workspace --all-targets` source gate, set and record `RUST_TEST_THREADS=1` instead because Criterion rejects libtest-only trailing arguments; the bounded receipt remains mandatory.
