...
- 2026-05-29 Codex handoff progress:
  ...
  - Use `scripts/m3_route_unified_profile.sh` on the restored pod to capture `[MOE_DUMP:*]`, `[vllm-moe-config]`, `[unified-prof]`, `[iter-prof]`, and `[bucket-prof]` in one scoped c=32 run; it sets `FERRUM_MOE_GRAPH=0` because route dumping syncs/copies GPU buffers and fails fast if route shape, Marlin config, or unified timing is missing.
