...
- Every long-running command must declare an expected duration, a hard deadline, and an observable progress signal before it starts. Poll the process and artifact directly; once the command exits, treat that result as final even if a client UI still says it is working.
- If a long-running command produces no new log bytes, CPU/GPU activity, or artifact progress by its deadline, inspect and classify it immediately. Do not keep waiting without a live process and a changing progress signal.
- Keep sub-agents off the critical path. Their work is advisory and may run in parallel, but the primary lane must not wait for a sub-agent when it can continue from repository state and artifacts.
- Use staged validation after a failure: exact reproducer, affected tests, backend smoke, then full gate only at a milestone. A single failed case must not restart an expensive full sweep until the focused case passes.
- During active long-lived goals, each 30-minute window must produce a diff, test result, commit, or diagnostic artifact. Otherwise stop the current approach, record the blocker, and choose a new bounded action.
...
- Never derive native thread or process counts directly from model size, memory capacity, request count, sequence count, or a user-supplied concurrency value. Every test or tool that starts workers must enforce an independent small upper bound before the first spawn, and partial startup failure must release and join every worker already started rather than leaving them blocked on an unreachable barrier.
- Validate newly added or materially changed concurrency/stress tests with `scripts/release/bounded_command.py` before including them in a broad local gate. A watchdog is secondary containment; source-level bounded worker creation is still required.
- Bound Rust test concurrency independently from build concurrency, but do not force libtest to a single thread. Use an explicit value from `2` through `8`, defaulting to `--test-threads=8` for bounded libtest-only commands. For the bench-inclusive `cargo test --workspace --all-targets` source gate, set and record `RUST_TEST_THREADS=8` instead because Criterion rejects libtest-only trailing arguments; the bounded receipt remains mandatory.
