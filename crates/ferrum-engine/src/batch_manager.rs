...
use ferrum_core::{
    BatchId, BatchInfo, BatchManager, BatchOutput, Error, GenerateOutput, InferenceRequest,
    RequestId, Result, Tensor,
};
...
impl BatchManager for ContinuousBatchManager {
    async fn create_batch(&self, requests: Vec<InferenceRequest>) -> Result<BatchId> {
        ...
        info!(
            "Creating new batch {:?} with {} requests",
            batch_id,
            requests.len()
        );
        ...
        self.active_batches
            .write()
            .insert(batch_id.clone(), active_batch);
    ...
    async fn add_to_batch(&self, batch_id: BatchId, request: InferenceRequest) -> Result<()> {
        ...
        debug!(
            "Added request to batch {:?}, new size: {}",
            batch_id,
            batch.requests.len()
        );
