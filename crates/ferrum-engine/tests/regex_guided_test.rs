//! Regex-guided decoding — end-to-end through the engine with a real
//! byte-level tokenizer.
//!
//! The `ByteTokenizer` here gives every byte 0..=vocab_size-2 its own token,
//! which is what `RegexGuidedProcessor` needs to compute real per-token
//! byte-by-byte DFA transitions (the default `MockTokenizer` returns `None`
//! from `token_text`, which defeats the mask).
//!
//! We bias the executor toward a non-matching token ('*' = 42) and let
//! the regex mask force the sampler into digit tokens only.
...
fn make_request(prompt: &str, pattern: &str, max_tokens: usize) -> InferenceRequest {
    ...
    req.sampling_params.response_format = ResponseFormat::Regex(pattern.to_string());
    req
}

/// The executor *wants* to emit '*' but the regex `[0-9]+` forbids it.
/// With `-INF` on non-digit logits and greedy sampling, every generated
/// token must be a digit byte (48..=57) until the stop token.
#[tokio::test]
async fn regex_digits_forces_digit_tokens() {
    let engine = make_engine();
    let req = make_request("hi", r"[0-9]+", 10);
    ...
    // No letters or '*' allowed — regex would have rejected them.
    for c in generated.chars() {
        assert!(
            c.is_ascii_digit() || c == (STOP_TOKEN as u8) as char,
            "unexpected char {c:?} (byte {}) in regex-guided output {generated:?}",
            c as u32,
        );
    }
}

/// Sanity check: without a regex, the same executor produces the '*'
/// tokens it was biased toward — confirms the regex path is what forces
/// the switch, not some incidental engine behaviour.
#[tokio::test]
