...
impl ConfigManager {
    ...
    fn parse_config(&mut self, raw: &serde_json::Value) -> Result<ModelDefinition> {
        let obj = raw.as_object().ok_or_else(|| {
            FerrumError::model("config.json root is not an object")
        })?;
        ...
        // Parse common fields
        let hidden_size = obj.get("hidden_size")
            .and_then(|v| v.as_u64())
            .unwrap_or(4096) as usize;
        
        let intermediate_size = obj.get("intermediate_size")
            .and_then(|v| v.as_u64())
            .or_else(|| obj.get("ffn_dim").and_then(|v| v.as_u64()))
            .unwrap_or(11008) as usize;
        
        let vocab_size = obj.get("vocab_size")
            .and_then(|v| v.as_u64())
            .ok_or_else(|| FerrumError::model("vocab_size not found in config"))? as usize;
        
        let num_hidden_layers = obj.get("num_hidden_layers")
            .and_then(|v| v.as_u64())
            .or_else(|| obj.get("n_layer").and_then(|v| v.as_u64()))
            .unwrap_or(32) as usize;
        
        let num_attention_heads = obj.get("num_attention_heads")
            .and_then(|v| v.as_u64())
            .or_else(|| obj.get("n_head").and_then(|v| v.as_u64()))
            .unwrap_or(32) as usize;
        
        let num_key_value_heads = obj.get("num_key_value_heads")
            .and_then(|v| v.as_u64())
            .map(|v| v as usize);
        
        let max_position_embeddings = obj.get("max_position_embeddings")
            .and_then(|v| v.as_u64())
            .or_else(|| obj.get("n_positions").and_then(|v| v.as_u64()))
            .unwrap_or(2048) as usize;
        
        let rope_theta = obj.get("rope_theta")
            .and_then(|v| v.as_f64())
            .or_else(|| obj.get("rotary_emb_base").and_then(|v| v.as_f64()));
        
        // Parse RoPE scaling
        let rope_scaling = obj.get("rope_scaling")
        ...
        let norm_eps = obj.get("rms_norm_eps")
        ...
        let attention_bias = obj.get("attention_bias")
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        
        let sliding_window = obj.get("sliding_window")
            .and_then(|v| v.as_u64())
            .map(|v| v as usize);
        
        // Detect activation
        let activation = obj.get("hidden_act")
            .and_then(|v| v.as_str())
            .map(|s| match s {
