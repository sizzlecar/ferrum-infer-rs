//! GenericModelExecutor — unified executor using ModelRunner<CpuBackend>.
//!
//! Replaces per-model executors (Qwen3ModelExecutor, CandleModelExecutor, etc.)
//! for decoder-only LLMs. The model-specific logic is entirely in TransformerConfig.
...
impl ModelExecutor for GenericModelExecutor {
    ...
    async fn prefill(&self, input: &PrefillInput) -> Result<PrefillOutput> {
        ...
        debug!("GenericExecutor prefill: {} tokens", tokens.len());

        let cache_id = self.gen_cache_id();

        let logits = {
            let mut runner = self.runner.lock();
            runner.prefill(&cache_id, &tokens)
        };
        ...
        let cfg = self.runner.lock().config().clone();
    ...
    async fn decode(&self, input: &DecodeInput) -> Result<DecodeOutput> {
        ...
        debug!("GenericExecutor decode: token={token}, pos={seq_len}");

        let logits = {
            let mut runner = self.runner.lock();
            runner.decode(&cache_id, token, seq_len as u32)
        };

        // Wrap logits as TensorRef: [1, vocab_size]
        let logits_tensor = candle_core::Tensor::new(&logits[..], &candle_core::Device::Cpu)
            .map_err(|e| FerrumError::model(format!("logits tensor: {e}")))?
            .unsqueeze(0)
            .map_err(|e| FerrumError::model(format!("unsqueeze: {e}")))?;
    ...
    fn release_cache(&self, cache_id: &str) {
        self.runner.lock().release(cache_id);
    }
    ...
    fn capabilities(&self) -> ExecutorCapabilities {
        ...
        ExecutorCapabilities {
            ...
            memory_requirements: MemoryRequirements {
                parameter_memory: (self.info.num_parameters * 4) as u64, // f32
