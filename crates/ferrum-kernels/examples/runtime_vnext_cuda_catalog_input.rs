...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if arguments.len() != 5 {
        return Err(format!(
            ...
            arguments
                .first()
                .map(String::as_str)
                .unwrap_or("runtime_vnext_cuda_catalog_input")
        )
        .into());
    }
    ...
    println!(
        "FERRUM RUNTIME VNEXT CUDA NATIVE CATALOG INPUT READY: provider={} capability={} provider_count={} capability_fingerprint={}",
        provider_catalog_path.display(),
        capability_catalog_path.display(),
        provider_catalog.providers.len(),
