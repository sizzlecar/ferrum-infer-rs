...
impl BackendPagedKv for CudaBackend {
    ...
    fn paged_varlen_attention_fa_flash_q8(
        ...
        let func = ctx.func(
            "paged_varlen_attn",
            ptx::PAGED_VARLEN_ATTENTION,
            "paged_varlen_attn_fa_flash_q8_f16",
        );
        let scale: f32 = 1.0 / (head_dim as f32).sqrt();
        let stream = ctx.stream.clone();
        ...
        unsafe {
            b.launch(LaunchConfig {
                ...
                block_dim: (256, 1, 1),
                shared_mem_bytes: 8 * 64 * 4,
            })
        }
        .map(|_| ())
        .map_err(|e| FerrumError::model(format!("paged_varlen_attn_fa_flash_q8: {e}")))
    }
    ...
    // ─────────── vLLM-layout paged write + paged_attention_v2 ───────────
