...
enum CudaRecurrentAttentionTopology {
    ...
    SingleTokenNormalizedPackedDecode,
    VarlenRecurrentScan,
}
...
impl CudaRecurrentAttentionTopology {
    fn select(
        ...
        if matches!(execution_form, GatedDeltaExecutionForm::RecurrentScan)
            ...
            && shape.tiled_delta
        {
            Self::SingleTokenNormalizedPackedDecode
        } else {
    ...
    const fn as_str(self) -> &'static str {
        match self {
            Self::SingleTokenNormalizedPackedDecode => "single_token_normalized_packed_decode",
    ...
    const fn compute_dispatches(self) -> u64 {
        match self {
            Self::SingleTokenNormalizedPackedDecode => 10,
    ...
    const fn transfer_commands(self) -> u64 {
        match self {
            Self::SingleTokenNormalizedPackedDecode => 0,
...
fn enqueue_attention(
    ...
    match launch.topology {
        CudaRecurrentAttentionTopology::SingleTokenNormalizedPackedDecode => {
            ...
            let packed_key = packed_decode_key_pointer(decode_mixed_qkv, shape)?;
            launch_qk_norm(
                stream,
                &functions.qk_norm,
                decode_mixed_qkv,
...
fn packed_decode_key_pointer(
    mixed_qkv: u64,
    shape: AttentionShape,
) -> Result<u64, CudaDeviceRuntimeError> {
    let query_bytes = shape
        .qk_features()
        .map_err(CudaDeviceRuntimeError::contract)?
        .checked_mul(ElementType::F32.size_bytes())
        .ok_or_else(|| {
            CudaDeviceRuntimeError::contract("attention packed QK byte offset overflows")
        })?;
    scratch_pointer(mixed_qkv, query_bytes)
}
...
mod tests {
    ...
    #[test]
    fn normalized_packed_decode_matches_varlen_cuda_state_and_output() {
        ...
        .expect("packed decode convolution");
        let new_key = packed_decode_key_pointer(new_mixed_qkv.device_ptr(&stream).0, shape)
            .expect("packed key pointer");
        launch_qk_norm(
            &stream,
            &functions.qk_norm,
            new_mixed_qkv.device_ptr(&stream).0,
