...
const MAX_ROWS: u64 = u16::MAX as u64;
    ...
    for participant in &invocation.participants()[1..] {
        ...
        if candidate.parts != matrix.parts
            ...
            || !candidate
                ...
                .iter()
                .zip(&matrix.regions)
                .all(|(a, b)| super::super::same_physical_region(a, b))
        {
...
fn launch_parts<'a>(
    ...
    while start < rows as u64 {
        ...
        let input = scratch_pointer(input, start * input_features as u64 * 2)?;
        let output = scratch_pointer(output, start * output_features as u64 * 2)?;
