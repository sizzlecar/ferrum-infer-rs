...
// supertraits whose impls haven't been extracted yet. Phase 1 split
// (#8): INT8 KV (`BackendInt8KvOps` + helpers + `KvCacheQuant`
// constructor) → `cuda/int8_kv.rs`. Phase 2: BackendCollective →
// `cuda/collective.rs`, BackendGraph + GraphSlotRaw + DECODE_GRAPHS
// helpers → `cuda/graph.rs`. Future phases extract the remaining
// supertraits (Marlin, PagedKv, MoeFused).
pub mod collective;
pub mod graph;
pub mod int8_kv;
