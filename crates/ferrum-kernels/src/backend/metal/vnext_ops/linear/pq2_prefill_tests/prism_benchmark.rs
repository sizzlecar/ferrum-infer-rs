...
use super::super::super::native_blocks::pq2_full_tiles_supported;
use super::*;
...
impl<'a> AlignedControl<'a> {
    fn new(device: &Device, source: &'a Fixture) -> Self {
        ...
        let mut input_values = vec![GUARD; ALIGNED_PREFIX_BYTES / 4];
    ...
    fn run_timed(
        ...
        encoder.set_buffer(0, Some(&self.input), ALIGNED_PREFIX_BYTES as u64);
    ...
    for (name, width, outputs) in [
        ...
        println!(
            ...
            serde_json::json!({
                ...
                "production_selection": "current selector, including full-tile M64 when eligible",
            })
        );
    }
}
