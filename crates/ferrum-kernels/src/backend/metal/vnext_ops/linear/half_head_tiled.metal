// Explicit half-operands Q6 head. Tile, loads and stores follow the legacy
// q6_k_gemm shader, but decode signed Q6 coefficients directly before half
// rounding. In particular, Inf * (q - 32) must not become Inf*q - Inf*32.
// This library is compiled with fast math disabled.
//
...
struct GemmQ6KParams {
    ...
    int strideC; // output row stride in F32 elements
};

// Returns a 4×4 tile (16 half-rounded weights) for il in [0, 16).
...
static inline void dequantize_q6_K(
    ...
    const float scale = float(d_all) * sc;
    ...
    FOR_UNROLL (int i = 0; i < 4; ++i) {
        ...
        reg[i][0] = scale * float(int(q & 255u) - 32);
        reg[i][1] = scale * float(int((q >> 8) & 255u) - 32);
        reg[i][2] = scale * float(int((q >> 16) & 255u) - 32);
        reg[i][3] = scale * float(int(q >> 24) - 32);
    }
}
