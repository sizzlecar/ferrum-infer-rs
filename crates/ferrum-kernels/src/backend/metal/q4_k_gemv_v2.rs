...
pub(crate) const F16_BATCHED_KERNEL_NAME: &str = "gemv_f16a_q4kw_v2_batched";
pub(crate) const F32_BATCHED_KERNEL_NAME: &str = "gemv_f32a_q4kw_v2_batched";
...
pub(crate) fn new_f16_batched_pipeline(device: &Device) -> Result<ComputePipelineState, String> {
    let library = device
        .new_library_with_source(SHADER_SRC, &CompileOptions::new())
        .map_err(|error| format!("compile shared Q4_K GEMV library: {error}"))?;
    let function = library
        .get_function(F16_BATCHED_KERNEL_NAME, None)
        .map_err(|error| format!("load shared Q4_K GEMV kernel: {error}"))?;
    device
        .new_compute_pipeline_state_with_function(&function)
        .map_err(|error| format!("build shared Q4_K GEMV pipeline: {error}"))
}

pub(crate) fn new_f32_batched_pipeline(device: &Device) -> Result<ComputePipelineState, String> {
    let library = device
        .new_library_with_source(SHADER_SRC, &CompileOptions::new())
        .map_err(|error| format!("compile shared Q4_K F32 GEMV library: {error}"))?;
    let function = library
        .get_function(F32_BATCHED_KERNEL_NAME, None)
        .map_err(|error| format!("load shared Q4_K F32 GEMV kernel: {error}"))?;
    device
        .new_compute_pipeline_state_with_function(&function)
        .map_err(|error| format!("build shared Q4_K F32 GEMV pipeline: {error}"))
}
