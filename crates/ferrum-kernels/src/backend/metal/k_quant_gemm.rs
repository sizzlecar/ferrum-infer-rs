//! Shared tiled Q4_K/Q5_K/Q6_K GEMM pipelines for F16 prefill execution.
...
impl MetalKQuantGemmPipelines {
    pub(crate) fn new(device: &Device) -> Result<Self, String> {
        let library = device
            ...
            .map_err(|error| format!("compile shared K-quant GEMM library: {error}"))?;
        let pipeline = |name: &str| {
            let function = library
                .get_function(name, None)
                .map_err(|error| format!("load shared K-quant GEMM kernel `{name}`: {error}"))?;
            device
                .new_compute_pipeline_state_with_function(&function)
                .map_err(|error| format!("build shared K-quant GEMM kernel `{name}`: {error}"))
        };
        Ok(Self {
