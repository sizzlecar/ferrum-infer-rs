//! Determinism and equivalence test for the GPU-side pair-id align kernel.
//! Builds a synthetic per-pair expert assignment, runs the kernel,
//! D2Hs the outputs, and compares against a Rust reference impl.
//!
//! Run with:
//!   cargo test -p ferrum-kernels --features cuda,vllm-moe-marlin --release \
...
// Matches the current c13-022 Qwen3.5 prefill shape that exposed the drift.
const NUM_EXPERTS: usize = 256;
const TOP_K: usize = 8;
const BATCH: usize = 61;
...
#[test]
fn moe_pair_id_align_is_bitwise_stable_and_matches_reference() {
    ...
    let mut expert_ids_dev = <ferrum_kernels::backend::cuda::CudaBackend as Backend>::alloc_typed(
    ...
    let mut sorted_dev = <ferrum_kernels::backend::cuda::CudaBackend as Backend>::alloc_typed(
        ferrum_kernels::backend::Dtype::U32,
        sorted_max,
    );
    let mut block_ids_dev = <ferrum_kernels::backend::cuda::CudaBackend as Backend>::alloc_typed(
        ferrum_kernels::backend::Dtype::U32,
        sorted_max / BLOCK_SIZE,
    );
    let mut total_dev = <ferrum_kernels::backend::cuda::CudaBackend as Backend>::alloc_typed(
        ferrum_kernels::backend::Dtype::U32,
        1,
    );
    ...
    let mut expected_sorted = vec![n_pairs as i32; sorted_max];
    expected_sorted[..used].copy_from_slice(&ref_sorted);
    let mut baseline: Option<(Vec<i32>, Vec<i32>, i32)> = None;

    for round in 0..4 {
        <ferrum_kernels::backend::cuda::CudaBackend as BackendMoeFused>::
            ...
            .expect("moe_align_block_size_pair_ids launch");

        let stream = ctx.stream.clone();
        let sorted_view = unsafe {
            sorted_dev
        ...
        stream
            .memcpy_dtoh(&sorted_view, sorted_host.as_mut_slice())
            .expect("dtoh sorted");
        ...
        let total_view = unsafe { total_dev.transmute::<i32>(1).expect("total transmute") };
        let mut total_host = vec![0i32; 1];
        stream
            .memcpy_dtoh(&total_view, total_host.as_mut_slice())
            .expect("dtoh total");
        stream.synchronize().expect("sync");

        assert_eq!(
            total_host[0], ref_total,
            "round {round}: total_tokens_post_pad mismatch"
        );
        assert_eq!(
            sorted_host, expected_sorted,
            "round {round}: pair ids must retain stable ascending order per expert"
        );
        assert_eq!(
            &block_ids_host[..total_blocks],
            &ref_block_ids,
            "round {round}: block ids mismatch"
        );

        let current = (
            sorted_host,
            block_ids_host[..total_blocks].to_vec(),
            total_host[0],
        );
        if let Some(baseline) = &baseline {
            assert_eq!(
                &current, baseline,
                "round {round}: output is not bitwise stable"
            );
        } else {
