...
// vendored for ferrum-infer-rs: dropped the torch::Tensor stub for the
// SM<8.0 case (we never compile for sub-Ampere). The CUDA_ARCH<8.0 #if
// branch above defines the kernel as empty so callers from extern C just
// abort with TORCH_CHECK at runtime if the device is too old.
...
// vendored for ferrum-infer-rs: dropped the torch::Tensor wrapper
// (`moe_wna16_marlin_gemm`) and the TORCH_LIBRARY_IMPL binding at the bottom.
// Instead, expose an `extern "C"` entry point that calls `marlin_mm<half>`
// directly with raw void* pointers from cudarc-managed buffers.
//
...
extern "C" int ferrum_vllm_marlin_moe_f16(
    ...
    const int* num_tokens_past_padded,    // [1] i32
    const float* topk_weights,            // [size_m, top_k] fp32 (or nullptr)
    int moe_block_size,                   // 8 / 16 / 32 / 48 / 64
    int top_k,
    int mul_topk_weights,                 // 0 or 1
    int is_ep,                            // 0 or 1
  ...
  MARLIN_NAMESPACE_NAME::marlin_mm<half>(
      ...
      /*perm=*/nullptr,
      /*a_tmp=*/nullptr,
      /*sorted_token_ids=*/(void*)sorted_token_ids,
      /*expert_ids=*/(void*)expert_ids,
      /*num_tokens_past_padded=*/(void*)num_tokens_past_padded,
      /*topk_weights=*/(void*)topk_weights,
...
#if 0
// vendored for ferrum-infer-rs: torch::Tensor wrapper retained as comment
// for reference only — it expects PyTorch to be in the link line.
...
#endif  // closes the vendored #if 0 above (torch::Tensor wrapper)
#endif  // closes #if defined(__CUDA_ARCH__) && __CUDA_ARCH__ < 800

// vendored for ferrum-infer-rs: dropped TORCH_LIBRARY_IMPL_EXPAND op
// registration (we don't bind to torch's C++ extension loader).
