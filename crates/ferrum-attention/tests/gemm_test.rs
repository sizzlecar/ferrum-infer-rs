...
mod gemm_tests {
    ...
    fn run_metal_gemm(pipes: &MetalPipelines, a: &[f32], b: &[f32], m: usize, n: usize, k: usize) -> Vec<f32> {
        ...
        let params_buf = pipes.device.new_buffer_with_data(
            &params as *const _ as *const std::ffi::c_void, 12,
    ...
    fn test_gemm_v2_aligned() {
        ...
        let max_diff = ref_c.iter().zip(metal_c.iter())
            .map(|(r, m)| (r - m).abs())
            .fold(0.0f32, f32::max);
    ...
    fn test_gemm_v2_perf() {
        ...
        eprintln!("GEMM {}x{}x{}: {:.3}ms/iter ({} iters in {:.1}ms)", m, n, k, per_iter, iters, elapsed.as_millis());
    }
}
