...
//!
//! Fixtures live in `tests/fixtures/chat_template/<model-slug>/` and were
//! captured from the upstream tokenizer implementations. This test reads only
//! checked-in files.
...
fn chat_template_goldens_match_transformers() {
    ...
    let mut model_dirs: Vec<PathBuf> = fs::read_dir(&root)
        .expect("checked-in chat-template fixture root is missing")
