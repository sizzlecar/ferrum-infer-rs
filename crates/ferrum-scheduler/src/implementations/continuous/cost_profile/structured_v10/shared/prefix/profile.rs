use super::*;
const ARTIFACT: &str = "ferrum.structured-prefix-v2-catalog";
...
pub struct StructuredProfileExportReceiptV12 {
...
#[derive(Debug, Clone)]
pub struct ImportedStructuredCatalogV12 {
...
pub fn export_structured_profile_v12(
    source_path: &Path,
    expected_source_sha256: [u8; 32],
    destination: &Path,
    declared_source_clock_errors_ns: &[u64],
    limits: &CostProfileLoadLimits,
) -> Result<StructuredProfileExportReceiptV12, CostProfileError> {
    limits.validate()?;
    ...
    let replayed = replay::replay_prefix_source(&source, limits)?;
    ...
    Ok(StructuredProfileExportReceiptV12 {
        schema_version: 12,
...
pub fn load_structured_profile_v12(
    ...
    Ok(ImportedStructuredCatalogV12 {
        children,
        file_sha256,
        source_sha256: declared.source_sha256,
