...
impl MetricsTracker {
    ...
    fn priority_wait_time(&self, priority: Priority) -> f64 {
        ...
        if let Some((count, total_wait)) = priority_stats.get(&priority) {
            if *count > 0 { *total_wait as f64 / *count as f64 } else { 0.0 }
        } else {
            0.0
        }
    }
...
impl PriorityScheduler {
    ...
    fn create_batch(&self, hint: BatchHint) -> Option<BatchPlan> {
        ...
        debug!("Creating priority batch {} with {} requests", batch_id, batch_requests.len());
        ...
        let highest_priority = batch_requests.iter()
...
impl Scheduler for PriorityScheduler {
    async fn submit(&self, request: InferenceRequest) -> Result<RequestId> {
        ...
        debug!("Submitting request {} with priority {:?} to priority scheduler", request_id, priority);
        ...
        info!("Request {} queued with priority {:?} at position {}", 
        ...
        if let Some(scheduled_req) = running_requests.remove(&request_id) {
            ...
            match response.finish_reason {
                Some(ferrum_types::FinishReason::Success) => {
                    ...
                    debug!("Request {} (priority {:?}) completed successfully", request_id, priority);
                }
                _ => {
                    self.failed_counter.fetch_add(1, Ordering::Relaxed);
                    warn!("Request {} (priority {:?}) completed with error: {:?}", 
    ...
    async fn update_priority(&self, request_id: RequestId, new_priority: Priority) -> Result<()> {
        debug!("Updating priority for request {} to {:?}", request_id, new_priority);

        let mut waiting_queue = self.waiting_queue.write();
        let mut request_map = self.request_map.write();
        
        // Check if request is in waiting queue
        if waiting_queue.change_priority(&request_id, RequestPriority::new(new_priority, chrono::Utc::now())).is_some() {
            ...
            if let Some(scheduled_req) = request_map.get_mut(&request_id) {
                ...
                info!("Updated priority for request {} to {:?}", request_id, new_priority);
                return Ok(());
            }
        }
    ...
    async fn preempt(&self, request_id: RequestId) -> Result<PreemptionResult> {
        ...
        if let Some(scheduled_req) = running_requests.get(&request_id) {
            ...
            if matches!(priority, Priority::Low | Priority::Medium) {
                ...
                warn!("Preempted request {} with priority {:?}", request_id, priority);
        ...
        Err(ferrum_types::FerrumError::scheduler(
            format!("Cannot preempt request {} (not found or high priority)", request_id)
        ))
    }
