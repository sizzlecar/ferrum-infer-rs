...
/// Actual shape of one admitted provider invocation. These values describe
/// work already selected for one batch; none is a global concurrency policy.
...
impl DynamicResourceShape {
    pub fn new(sequences: u32, tokens: u64, pages: u64) -> Result<Self, VNextError> {
...
impl DynamicResourceDemand {
    ...
    pub fn evaluate_bytes(&self, shape: DynamicResourceShape) -> Result<u64, VNextError> {
        self.validate()?;
    ...
    pub fn minimum_shape(&self) -> DynamicResourceShape {
    ...
    pub fn theoretical_maximum_shape(&self) -> DynamicResourceShape {
        match self {
...
impl DynamicResourceDescriptor {
    ...
    fn new(
        ...
        descriptor.evaluate_request_bytes(descriptor.demand.minimum_shape())?;
        descriptor.evaluate_request_bytes(descriptor.demand.theoretical_maximum_shape())?;
        Ok(descriptor)
    }
    ...
    pub fn evaluate_logical_request_bytes(
        ...
        self.demand.evaluate_bytes(shape)
    }
    ...
    pub fn evaluate_request_bytes(&self, shape: DynamicResourceShape) -> Result<u64, VNextError> {
        quantize_storage_bytes(
            self.evaluate_logical_request_bytes(shape)?,
            self.alignment_bytes,
    ...
    pub fn minimum_request_bytes(&self) -> Result<u64, VNextError> {
        self.evaluate_request_bytes(self.demand.minimum_shape())
    }

    pub fn theoretical_maximum_request_bytes(&self) -> Result<u64, VNextError> {
        self.evaluate_request_bytes(self.demand.theoretical_maximum_shape())
    }
...
impl ProviderWorkspaceRequirement {
    ...
    pub fn evaluate_bytes(&self, shape: DynamicResourceShape) -> Result<u64, VNextError> {
        align_up(
            self.size_formula.evaluate_bytes(shape)?,
            self.alignment_bytes,
        )
    }

    pub fn minimum_bytes(&self) -> Result<u64, VNextError> {
        self.evaluate_bytes(self.size_formula.minimum_shape())
    }

    pub fn theoretical_maximum_bytes(&self) -> Result<u64, VNextError> {
        self.evaluate_bytes(self.size_formula.theoretical_maximum_shape())
    }
...
mod tests {
    ...
    fn fixed_block_storage_quantizes_logical_demand_and_maxima() {
        ...
        assert_eq!(
            descriptor
                .evaluate_logical_request_bytes(DynamicResourceShape::new(1, 1, 1).expect("shape"))
                .expect("logical"),
            1
        );
