...
mod dynamic_pool_tests {
    ...
    fn pool_catalog(
        ...
        let compatibility_bytes =
            serde_json::to_vec(&canonical_json(compatibility.clone())).unwrap();
    ...
    fn harness(
        ...
        let provisioned = ProvisionedPlanResources::new(StaticProvisioning::NoStatic(NoStatic {
            maintenance_controller: DynamicPoolMaintenanceController::new(Arc::clone(
                &dynamic_pools,
            )),
    ...
    fn lifecycle_gate_linearizes_maintenance_before_close() {
        ...
        let close_attempting =
            Arc::new(std::sync::Barrier::new(DYNAMIC_POOL_CONCURRENT_WORKERS + 1));
...
mod sequence_session_frame_tests {
    ...
    fn cancel_after_step_rejects_new_invocation_with_zero_partial_flights() {
        ...
        let mut frames =
            acquire_session_frames(&[first.clone(), cancelled.clone()], step(1)).unwrap();
        let candidates = vec![
    ...
    fn participant_flight_drop_poison_closes_a_phase_mismatch() {
        ...
        begin_participant_flights_dispatch(&mut prepared).unwrap();
        {
            ...
            *active.participant_flights.values_mut().next().unwrap() =
