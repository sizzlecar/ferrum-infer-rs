...
use ferrum_core::{Result, BlockId, RequestId};
use crate::types::*;
...
pub trait CacheAllocator: Send + Sync {
    ...
    fn allocate(&self, request: &AllocationRequest, free_blocks: &[BlockId]) -> Result<Vec<BlockId>>;
    
    /// Suggest blocks to free for a new allocation
    fn suggest_eviction(&self, request: &AllocationRequest, all_blocks: &[BlockId]) -> Vec<BlockId>;
