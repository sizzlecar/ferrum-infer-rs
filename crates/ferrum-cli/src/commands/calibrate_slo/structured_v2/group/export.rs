...
pub(super) fn export_and_inspect(
    ...
    if capture.shared_source.is_some() {
        return export_shared_and_inspect(
            session, capture, source, report, artifacts, &limits, prefix_v5,
        );
    }
...
fn export_shared_and_inspect(
    ...
    let errors = capture
        ...
        .collect::<Vec<_>>();
    let children = if prefix_v5 {
        let exported = ferrum_scheduler::implementations::continuous::cost_profile::export_structured_profile_v12(
            &first.source_path, first.source_sha256, &capture.catalog, &errors, limits,
        ).map_err(|e| FerrumError::config(format!("group profile12 export: {e}")))?;
        report.exported_prefix_profile = Some(exported);
        &report.exported_prefix_profile.as_ref().unwrap().children
    } else {
        let exported =
        ...
        &report.exported_shared_profile.as_ref().unwrap().children
    };
    if children.len() != source.children.len() {
    ...
    artifacts.record(&serde_json::json!({"schema_version":1,"event":"structured_v2_shared_profile_exported","source_schema":if prefix_v5 {5}else{4},"receipt":report.exported_shared_profile,"prefix_receipt":report.exported_prefix_profile}))?;
    let receipt = session.inspect_structured_cost_profile_v2(&capture.catalog)?;
    let v2 = receipt
        .structured_whole_wave_v2
        .as_ref()
        .ok_or_else(|| FerrumError::internal("shared loader returned no V2 receipt"))?;
    let expected_kind = if prefix_v5 {
        ferrum_types::SloStructuredArtifactKindV2::PrefixCatalogV12
    } else {
        ferrum_types::SloStructuredArtifactKindV2::SharedCatalogV11
    };
    if v2.artifact_kind != expected_kind || v2.child_count != capture.children.len() {
        return Err(FerrumError::internal(
