#!/usr/bin/env bash
#
# Scoped M3 A/B for vLLM-layout varlen attention split-K.
#
# Usage on a GPU pod from the repo root:
#   MODEL_DIR=/workspace/hf-cache/hub/models--Qwen--Qwen3-30B-A3B-GPTQ-Int4/snapshots/<rev> \
#   OUT_ROOT=/workspace/m3-vllm-varlen-splitk-ab \
#   REPEATS=1 \
#   bash scripts/m3_vllm_varlen_splitk_ab.sh
#
# Optional:
#   BUILD=1 bash scripts/m3_vllm_varlen_splitk_ab.sh
...
OUT_ROOT="${OUT_ROOT:-/workspace/m3-vllm-varlen-splitk-ab-$(date +%Y%m%d_%H%M%S)}"
REPEATS="${REPEATS:-1}"
PORT_BASE="${PORT_BASE:-18240}"
...
run_case splitk "$PORT_BASE" FERRUM_VLLM_VARLEN_SPLIT_K=1
run_case default "$((PORT_BASE + 1))" FERRUM_VLLM_VARLEN_SPLIT_K=0
...
if len(rows) == 2 and rows[0][1] and rows[1][1]:
    base, candidate = rows
    print(f"DELTA {candidate[0]} vs {base[0]}: {(candidate[1] / base[1] - 1.0) * 100.0:.2f}%")
PY
