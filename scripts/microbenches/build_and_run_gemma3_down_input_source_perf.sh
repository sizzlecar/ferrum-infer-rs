#!/usr/bin/env bash
#
# Build + run the standalone Gemma3 down_proj input-source native-CUDA probe.
#
# This bypasses Cargo and model loading. It compiles only the probe plus the
# product CUDA kernels needed by the MLP source chain:
...
NVCC="${NVCC:-nvcc}"
ARCH="${ARCH:-sm_89}"
SRC="scripts/microbenches/gemma3_down_input_source_perf.cu"
OUT_BIN="${OUT_BIN:-/tmp/gemma3_down_input_source_perf}"
KERNEL_SRCS=(
