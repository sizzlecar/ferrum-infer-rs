// C ABI bridge from Ferrum to vLLM FlashAttention-2.
//
// This shared object is intentionally built out-of-tree by
// build_fa2_ferrum_shim.sh. Ferrum dlopens it only when
// FERRUM_FA2_DIRECT_FFI=1, keeping Torch/Python/FA2 extension dependencies out
// of the normal binary.
...
extern "C" int ferrum_fa2_paged_varlen_fwd(
    ...
    try {
        ...
        // For paged KV, block_table resolves the physical K/V pages and
        // seqused_k carries the per-sequence final KV length. The FA2 kernel
        // does not need cumulative K offsets when block_table is set.
        ...
        flash::run_mha_fwd(params, reinterpret_cast<cudaStream_t>(stream),
                           /*force_split_kernel=*/true);
        const cudaError_t launch_err = cudaPeekAtLastError();
        if (launch_err != cudaSuccess) {
            set_err(err_buf, err_buf_len, "FA2 launch failed: %s",
