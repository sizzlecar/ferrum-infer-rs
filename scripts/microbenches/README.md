...
| `build_and_run_gemma3_marlin_cache_policy_perf.sh` | Native A/B for the Gemma3 tail-MLP chain with product Marlin B-weight `cp.async.cg` versus the compile-time `FERRUM_MARLIN_CP_ASYNC_EVICT_FIRST=1` cache-policy variant. Use before wiring any Marlin cache-policy change into defaults. |
