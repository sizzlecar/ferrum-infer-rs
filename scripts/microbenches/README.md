...
| `fa2_ferrum_source_shim.cu` | Legacy source-built FA2 C ABI shim with no vLLM/Torch runtime link. Build it with `build_fa2_ferrum_source_shim.sh` only for historical A/B replay; the product `fa2-source` path now builds the in-repo kernel under `crates/ferrum-kernels/kernels/fa2_source/`. |
