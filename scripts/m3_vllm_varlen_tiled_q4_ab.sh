#!/usr/bin/env bash
#
# Scoped M3 A/B for the opt-in vLLM-layout varlen attention Q-tiled kernel.
#
# Usage on a GPU pod from the repo root:
#   MODEL_DIR=/workspace/hf-cache/hub/models--Qwen--Qwen3-30B-A3B-GPTQ-Int4/snapshots/<rev> \
#   OUT_ROOT=/workspace/m3-vllm-varlen-tiled-q4-ab \
#   REPEATS=1 \
#   bash scripts/m3_vllm_varlen_tiled_q4_ab.sh
#
# Optional:
#   BUILD=1 bash scripts/m3_vllm_varlen_tiled_q4_ab.sh
...
OUT_ROOT="${OUT_ROOT:-/workspace/m3-vllm-varlen-tiled-q4-ab-$(date +%Y%m%d_%H%M%S)}"
REPEATS="${REPEATS:-1}"
PORT_BASE="${PORT_BASE:-18280}"
...
run_case tiled_q4 "$PORT_BASE" FERRUM_VLLM_VARLEN_TILED_Q4=1
run_case default "$((PORT_BASE + 1))" FERRUM_VLLM_VARLEN_TILED_Q4=0
...
if rows.get("tiled_q4", (None, None))[0] and rows.get("default", (None, None))[0]:
    tiled = rows["tiled_q4"][0]
    default = rows["default"][0]
    print(f"DELTA tiled_q4 vs default: {(tiled / default - 1.0) * 100.0:.2f}%")
PY
