...
def validate_manifest(source: Path) -> dict[str, Any]:
    ...
    require(
        manifest.get("server") == {"args": ["--backend", "cuda"], "mode": "start"},
        "manifest must use dynamic typed CUDA defaults",
    )
    require(
...
def validate_identity(source: Path, expected_git_sha: str | None) -> tuple[dict[str, Any], Path, dict[str, Any]]:
    ...
    require("--max-num-seqs" not in server_argv, "sentinel must exercise dynamic admission defaults")
