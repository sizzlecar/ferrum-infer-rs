#!/usr/bin/env bash
#
# Scoped M3 A/B for active-decode-only prefill chunking.
#
# This candidate keeps initial empty-queue prefills full-size, but chunks new
# prefills that are admitted while decode requests are already active. It is
# meant to reduce mixed-prefill ITL tails without the TTFT cost seen from
# chunking every prefill.
#
# Usage on a GPU pod from the repo root:
#   MODEL_DIR=/workspace/hf-cache/hub/models--Qwen--Qwen3-30B-A3B-GPTQ-Int4/snapshots/<rev> \
#   OUT_ROOT=/workspace/m3-active-prefill-chunk-ab \
#   REPEATS=1 \
#   ACTIVE_PREFILL_CHUNK=64 \
#   bash scripts/m3_active_prefill_chunk_ab.sh
#
# Optional:
#   BUILD=1 bash scripts/m3_active_prefill_chunk_ab.sh
...
echo "REPEATS=$REPEATS ACTIVE_PREFILL_CHUNK=$ACTIVE_PREFILL_CHUNK"

{
    ...
    echo "active_prefill_chunk=$ACTIVE_PREFILL_CHUNK"
...
run_case active_chunk "$PORT_BASE" FERRUM_ACTIVE_DECODE_PREFILL_CHUNK="$ACTIVE_PREFILL_CHUNK"
run_case default "$((PORT_BASE + 1))" FERRUM_ACTIVE_DECODE_PREFILL_CHUNK=0
...
if rows.get("active_chunk", (None, None))[0] and rows.get("default", (None, None))[0]:
    active = rows["active_chunk"][0]
    default = rows["default"][0]
    print(f"DELTA active_chunk vs default: {(active / default - 1.0) * 100.0:.2f}%")
PY
