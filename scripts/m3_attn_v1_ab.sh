#!/usr/bin/env bash
#
# Thin wrapper around scripts/m3_ab_runner.py for the M3 short-context
# vLLM paged-attention v1/v2 A/B gate.
...
port_base = int(os.environ.get("PORT_BASE", "18152"))
config = {
    "name": "m3-attn-v1-ab",
    ...
    "cases": [
        {
            "name": "v1_short",
            "port": port_base,
            "env": {"FERRUM_VLLM_PAGED_ATTN_V1_SHORT": "1"},
        },
        {
            "name": "v2_forced",
            "port": port_base + 1,
            "env": {"FERRUM_VLLM_PAGED_ATTN_V1_SHORT": "0"},
        },
    ],
}
