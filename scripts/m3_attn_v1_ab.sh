#!/usr/bin/env bash
#
# Run the M3 short-context vLLM paged-attention v1/v2 A/B gate on a GPU pod.
#
# This script is intentionally scoped to one lever:
#   default short-context v1 path vs forced old v2 path
#
# Usage on a Vast pod from the repo root:
#   MODEL_DIR=/workspace/hf-cache/hub/models--Qwen--Qwen3-30B-A3B-GPTQ-Int4/snapshots/<rev> \
#   OUT_ROOT=/workspace/m3-attn-v1-ab \
#   REPEATS=3 \
#   bash scripts/m3_attn_v1_ab.sh
#
# Optional:
#   BUILD=1 bash scripts/m3_attn_v1_ab.sh
...
OUT_ROOT="${OUT_ROOT:-/workspace/m3-attn-v1-ab-$(date +%Y%m%d_%H%M%S)}"
REPEATS="${REPEATS:-1}"
PORT_BASE="${PORT_BASE:-18152}"
...
print(f"RESULT throughput={mean} ci95={ci95} tpot_p50={tpot} file={path}")
PY
...
for path in sorted(root.glob("*_c*_n*/bench.json")):
    data = json.load(open(path))
    throughput = data.get("output_throughput_tps") or {}
    rows.append((path.parent.name, throughput.get("mean", data.get("output_throughput")), throughput.get("ci95_hw")))
for name, mean, ci95 in rows:
    print(f"SUMMARY {name}: throughput={mean} ci95={ci95}")
if len(rows) == 2 and rows[0][1] and rows[1][1]:
    first, second = rows
    print(f"DELTA {first[0]} vs {second[0]}: {(first[1] / second[1] - 1.0) * 100.0:.2f}%")
PY
