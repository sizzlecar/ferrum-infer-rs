...
CASES = (
    ...
    ModelCase(
        ...
        moe=True,
        cells=(Cell(concurrency=16, prompts=32, baseline_tps=79.2),),
    ),
)
...
def chat_check(port: int, case: ModelCase, out_dir: Path) -> dict[str, Any]:
    ...
    multiturn_payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "Answer the final user question using the conversation context.",
            },
            {
                "role": "user",
            ...
            {
                "role": "user",
                "content": "What city did the assistant just name?",
            },
        ],
        "max_tokens": 16,
        "temperature": 0,
    ...
    try:
        ...
        content = body[:300]
    multiturn_pass = status == 200 and "paris" in content.lower()
    result["multiturn"] = {
        "status": status,
...
def markdown_summary(report: dict[str, Any]) -> str:
    out = ["# Metal README Regression - 2026-06-01", ""]
    ...
    out.append("")
    out.append("| Model | Correctness | Multi-turn | c | README tok/s | Current tok/s | Ratio | Completed | Gate |")
    out.append("|---|---:|---:|---:|---:|---:|---:|---:|---|")
    for model in report["models"]:
        ...
        for cell in model["cells"]:
            ...
            if isinstance(tps, (int, float)):
                out.append(
                    f"| {model['label']} | {correctness} | {multiturn} | {cell['concurrency']} | "
    ...
    for model in report["models"]:
        out.append(
            ...
            f"multi-turn: `{model['chat']['multiturn']['content']}`"
        )
    out.append("")
    out.append("Notes:")
