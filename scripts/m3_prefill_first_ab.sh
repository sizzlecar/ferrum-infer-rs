#!/usr/bin/env bash
#
# Thin wrapper around scripts/m3_ab_runner.py for prefill-first admission A/B.
...
export PREFILL_FIRST_TARGET FEATURES BUILD="${BUILD:-0}"
...
port_base = int(os.environ.get("PORT_BASE", "18420"))
config = {
    "name": "m3-prefill-first-ab",
    ...
    "cases": [
        {
            "name": "prefill_first",
            "port": port_base,
            "env": {"FERRUM_SCHED_PREFILL_FIRST_UNTIL_ACTIVE": os.environ["PREFILL_FIRST_TARGET"]},
        },
        {
            "name": "default",
            "port": port_base + 1,
            "env": {"FERRUM_SCHED_PREFILL_FIRST_UNTIL_ACTIVE": "0"},
        },
    ],
}
